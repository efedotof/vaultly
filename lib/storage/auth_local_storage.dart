...
import 'package:vaulth_app/server/model/auth/auth_response.dart';
...
class AuthLocalStorage {
  ...
  Future<void> saveAuthData(AuthResponse response) async {
    ...
    await SecureStorageAdapter.write(
      key: _storageUsedKey,
      value: (response.storageUsed ?? 0).toString(),
    );
    await SecureStorageAdapter.write(
      key: _storageLimitKey,
      value: (response.storageLimit ?? 0).toString(),
    );
    await SecureStorageAdapter.write(
      key: _rolesKey,
      value: (response.roles ?? const <String>{}).join(','),
    );
    await SecureStorageAdapter.write(
      key: _totpEnabledKey,
      value: (response.totpEnabled ?? false).toString(),
    );
  }
  ...
  Future<AuthResponse?> loadAuthData() async {
    ...
    final totpEnabledStr = await SecureStorageAdapter.read(
      key: _totpEnabledKey,
    );
    ...
    return AuthResponse(
      ...
      email: email,
      username: username,
      storageUsed: int.tryParse(storageUsedStr ?? '') ?? 0,
      storageLimit: int.tryParse(storageLimitStr ?? '') ?? 0,
      roles: (rolesStr == null || rolesStr.isEmpty)
          ? <String>{}
          : rolesStr.split(',').toSet(),
      totpEnabled: totpEnabledStr == 'true',
    );
  }
  ...
  Future<void> clearAuthData() async {
    ...
    await SecureStorageAdapter.delete(key: _totpEnabledKey);
