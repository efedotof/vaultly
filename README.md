...
> ⚠️ `S3_ACCESS_KEY` / `S3_SECRET_KEY` используются одновременно как root-креды MinIO и как ключи S3 для приложения. Задайте их один раз - менять после первого запуска нельзя без сброса volume MinIO (`docker compose down -v`).
